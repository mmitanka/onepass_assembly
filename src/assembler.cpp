...
void Assembler::work() {
    ...
    for(unsigned long i=0; i<count;i++) {
        ...
        if(curT.getToken_type()==Token_type::LABEL) {//keep that in mind
            TabSymb *tab=TabSectionHeader::getTabSymb();//nzm da li ce ovo sa ovim pokazivacima da radi
            tok.dealWithLabel(tab,curT,cur_sec, i, loc_cnt);
        ...
        switch(curT.getToken_type()) {
            ...
            case Token_type::SCOPE_DIRECTIVE:{
                ...
                cout<<"nasla sam prvi token"<<curT.getVal()<<endl;
                if(cntt==0) {
                //error neki 
            ...
            case Token_type::MEMORY_DIRECTIVE:{
                ...
                if(curT.getVal()==".skip") {
                    ...
                    for(unsigned j=0; j<cnt; j++) {
                        ...
                        long bytes= stol(curT.getVal(),NULL,0);
                        TabSection *tab=nullptr;
                        tab=TabSectionHeader::getTabSec(cur);
                        loc_cnt+=(tok.dealWithMemory(tab,curT,mem,bytes, cur,Rel_type::R_16));
                        tab=nullptr;
                    }
                }else {
                    //some error 
            ...
            case Token_type::SECTION_DIRECTIVE: {
                cout<<"nasla sam section token provera  "<<" "<<curT.getVal()<<endl;
            ...
            case Token_type::INSTRUCTION: {
                ...
                if(cur!=".text" || cur!=".TEXT" || (cur==".rodata" || cur==".RODATA" || cur==".bss" || cur==".BSS" || cur==".data" || cur==""
                || cur=="UND" || cur==".DATA")) {
                    string str="Can not use instruction in this sections";
                    throw str;
                }
