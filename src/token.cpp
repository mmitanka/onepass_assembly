...
Token Token::resolveToken(string t) {
    ...
    regex("^(((push|PUSH|pop|POP|xchg|XCHG|mov|MOV|add|ADD|sub|SUB|mul|MUL|div|DIV|cmp|CMP|not|NOT|and|AND|or|OR|xor|XOR|tets|TEST|shl|SHL|shr|SHR)(b|B|w|W){0,1})|(halt|HALT|iret|IRET|ret|RET|int|INT|call|CALL|jmp|JMP|jeq|JEQ|jne|JNE|jgt|JGT))$"))) {
...
uint16_t Token::dealWithSection(TabSymb* tab, string old_section_name, uint16_t loc_cnt, string new_symb_name) {
    cout<<"usla u deal with section"<<endl;
    if(old_section_name==""){
